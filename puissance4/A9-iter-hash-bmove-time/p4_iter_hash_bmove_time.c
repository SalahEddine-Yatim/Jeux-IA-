...
{
  int ev;
  int v_inf,v_sup,bmove=-1,lmove=-1,best;
  bool do_bmove,do_retrieve=false;
  ...
  /*  check_hash(hv);*/
  int a=alpha,b=beta;
  if (retrieve_v_hash(hv,maxdepth-depth,&v_inf,&v_sup,&bmove)) {
  ...
  /* Test first if there is a winning move; 
     ...
     If retrieve has been done, we don't need to test for a win
  */
  if ((!do_retrieve)||(depth==maxdepth)) {
    int v = eval(color,tab,first,&best);
    ev=v;
    if (abs(v)==WIN) {
      if (v>0) v=v-depth;else v=v+depth;
      if (depth==base) best_move= best;
      store_v_hash_both(hv,v,maxdepth-depth,base,best);
      return v;
    }
  }
...
/* If no move is a win and there is only one move left, it's a draw */
  if (depth==(SIZEX*SIZEY-1)) {
    /* if depth=base we need to know which move it is */
    if (depth==base) {
      for (int x=0;x<SIZEX;x++)
	if (first[x]!=SIZEY) {best_move=x;break;}
    }
    return 0;
  }
  ...
  if ((bmove!=-1)&&(first[bmove]<SIZEY)) do_bmove = true; else do_bmove=false;

  int g;
  if (color==WHITE) g=-MAXV; else g=MAXV;
  for (int ix=0;ix<SIZEX;ix++) {
    int x;
    if (do_bmove) {
      if (ix==0) x=bmove;
      else {x=indx[ix-1];if (bmove==x) {x=indx[ix];do_bmove=false;}}
    }
    else x=indx[ix];
    int y=first[x];
    if (y<SIZEY) {
      ...
      int v;
      if (color==WHITE) {
	v=alpha_beta(a,b,-color,depth+1,tab,first,maxdepth,base,
		     hv^hashesw[x][y]);
      }
      else {
	v=alpha_beta(a,b,-color,depth+1,tab,first,maxdepth,base,
  ...
  store_v_hash(hv,alpha,beta,g,maxdepth-depth,base,lmove);
  return g;
  }
